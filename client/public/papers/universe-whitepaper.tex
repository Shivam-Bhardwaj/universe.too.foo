...
\author{
    Shivam Bhardwaj\\
    HELIOS Project\\
    \texttt{https://github.com/helios-project/universe}
}
...
\begin{enumerate}
    \item \textbf{Neural scene representations} (3D Gaussian Splats) trained directly from astronomical catalogs
    \item \textbf{Mission-grade orbital mechanics} integrated with real-time rendering
    \item \textbf{GPU-rendered frame streaming} to browsers without client-side GPU requirements
    \item \textbf{Numerical precision} across 20 orders of magnitude in spatial scale
\end{enumerate}

\subsection{Key Contributions}

\begin{itemize}
    \item \textbf{Heliocentric Logarithmic Grid (HLG):} Spatial partitioning using logarithmic radial shells matching astronomical density falloff
    \item \textbf{Astronomical-Scale Neural Rendering:} First application of 3D Gaussian Splatting to heliocentric visualization with logarithmic depth
    \item \textbf{tch-rs CUDA Training Backend:} Differentiable rasterizer using raw LibTorch for CUDA-accelerated optimization
    \item \textbf{Zero-Install Cloud Astronomy:} Browser-accessible exploration via WebRTC streaming
\end{itemize}
